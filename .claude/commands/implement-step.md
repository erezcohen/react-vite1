...
- [name]: extract name of file from $1
- [step]: $2. If only one argument was given then set to [step] the next number after the last one documented in dev/implementations/[name]. If no such file exists assume [step] = 1.

## Task

Implement the step number [step] as defined in $1, and nothing else. Think hard.
